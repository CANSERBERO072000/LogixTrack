...
export default function DashboardPage() {
  ...
  const procesarArchivo = useCallback(async (file: File) => {
    ...
    // Trae guías ya existentes en el sistema (histórico) para contra-validar
    ...
    const filasConciliadas: FilaConciliacion[] = filasValidas.map((f) => {
      ...
      if (!existente) {
        // Está en Excel pero no en el sistema físico -> falta escaneo/entrega
        return { ...f, estado: 'faltante' as const }
      }
    ...
    // Detecta sobrantes: paquetes físicos que no aparecen en este manifiesto
    // (se simplifica aquí; en producción se cruza contra el rango de guías del manifiesto)

    setFilas(filasConciliadas)

    // Registra el manifiesto cargado
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4">
        <h1 className="text-xl font-semibold text-slate-900">Portal Coordinador</h1>
        <p className="text-sm text-slate-500">Conciliación automática de manifiestos Aranda</p>
      </header>
      ...
      <main className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`rounded-2xl border-2 border-dashed p-10 flex flex-col items-center justify-center text-center transition-colors bg-white
            ${arrastrando ? 'border-blue-600 bg-blue-50' : 'border-slate-300'}`}
        >
          {procesando ? (
            <>
            ...
            <>
              ...
              <p className="text-sm font-medium text-slate-700">
                Arrastra el Excel de Aranda aquí, o
              </p>
        ...
        {erroresParseo.length > 0 && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <ul className="text-xs text-amber-700 space-y-1 max-h-32 overflow-y-auto">
              {erroresParseo.slice(0, 10).map((e, i) => (
        ...
        {filas.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              <TarjetaSemaforo
                label="Conciliado" valor={conteos.conciliado}
                color="emerald" icon={<CheckCircle2 className="w-5 h-5" />}
                activo={filtro === 'conciliado'} onClick={() => setFiltro(filtro === 'conciliado' ? 'todos' : 'conciliado')}
              />
              <TarjetaSemaforo
                label="Faltante" valor={conteos.faltante}
                color="rose" icon={<XCircle className="w-5 h-5" />}
                activo={filtro === 'faltante'} onClick={() => setFiltro(filtro === 'faltante' ? 'todos' : 'faltante')}
              />
              <TarjetaSemaforo
                label="Sobrante/Imprevisto" valor={conteos.sobrante}
                color="amber" icon={<AlertTriangle className="w-5 h-5" />}
                activo={filtro === 'sobrante'} onClick={() => setFiltro(filtro === 'sobrante' ? 'todos' : 'sobrante')}
              />
            </div>
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-slate-700">
                    {filasFiltradas.length} paquetes {filtro !== 'todos' ? `(${filtro})` : ''}
                  </span>
                </div>
                <button
                  onClick={() => exportarCierreAranda(filas)}
                  className="flex items-center gap-1.5 text-sm font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-lg px-3.5 py-2 transition-colors"
                >
...
function TarjetaSemaforo({ label, valor, color, icon, activo, onClick }: {
  label: string; valor: number; color: 'emerald' | 'rose' | 'amber'
  icon: React.ReactNode; activo: boolean; onClick: () => void
}) {
  const colores = {
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left rounded-2xl border p-5 bg-white transition-shadow hover:shadow-sm ${activo ? 'ring-2 ring-offset-1 ring-slate-300' : ''}`}
    >
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center border mb-3 ${colores[color]}`}>
        {icon}
      </div>
...
function Badge({ estado }: { estado: 'conciliado' | 'faltante' | 'sobrante' }) {
  ...
  return (
    <span className={`inline-flex text-xs font-medium px-2 py-1 rounded-md border ${estilos[estado]}`}>
      {etiquetas[estado]}
    </span>
  )
}
