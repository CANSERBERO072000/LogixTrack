-- =====================================================================
-- SISTEMA DE CONTROL LOGÍSTICO — schema.sql (BASE)
-- Ejecutar completo en Supabase SQL Editor, ANTES de schema_extension.sql
-- =====================================================================
...
do $$ begin
  create type estado_paquete as enum ('pendiente', 'en_ruta', 'entregado', 'fallido', 'en_bodega', 'reasignado', 'conciliado', 'faltante', 'sobrante');
exception when duplicate_object then null; end $$;
...
create trigger trg_paquetes_updated_at before update on public.paquetes for each row execute function public.set_updated_at();
...
create trigger trg_inconsistencia_estado after insert on public.inconsistencias for each row execute function public.marcar_estado_por_inconsistencia();
...
select
  ...
  round(100.0 * count(*) filter (where p.estado = 'entregado') / nullif(count(*),0), 2) as porcentaje_otif
...
select
  m.id as manifiesto_id, m.nombre_archivo,
...
select
  p.id as paquete_id, p.guia, e.tipo, e.timestamp_captura, e.latitud, e.longitud, e.storage_path,
  pf.nombre_completo as usuario, pf.role as usuario_role
from public.paquetes p
...
create policy "perfiles_select_propio_o_admin" on public.perfiles for select using (id = auth.uid() or public.current_role_name() in ('admin','executive'));

drop policy if exists "perfiles_update_propio" on public.perfiles;
create policy "perfiles_update_propio" on public.perfiles for update using (id = auth.uid() or public.current_role_name() = 'admin');

drop policy if exists "rutas_admin_full" on public.rutas;
create policy "rutas_admin_full" on public.rutas for all using (public.current_role_name() in ('admin','executive')) with check (public.current_role_name() in ('admin','executive'));

drop policy if exists "rutas_driver_propias" on public.rutas;
create policy "rutas_driver_propias" on public.rutas for select using (driver_id = auth.uid());

drop policy if exists "rutas_warehouse_lectura" on public.rutas;
create policy "rutas_warehouse_lectura" on public.rutas for select using (public.current_role_name() = 'warehouse');

drop policy if exists "manifiestos_admin_full" on public.manifiestos_aranda;
create policy "manifiestos_admin_full" on public.manifiestos_aranda for all using (public.current_role_name() in ('admin','executive')) with check (public.current_role_name() in ('admin','executive'));

drop policy if exists "paquetes_admin_full" on public.paquetes;
create policy "paquetes_admin_full" on public.paquetes for all using (public.current_role_name() in ('admin','executive')) with check (public.current_role_name() in ('admin','executive'));

drop policy if exists "paquetes_driver_asignados" on public.paquetes;
create policy "paquetes_driver_asignados" on public.paquetes for select using (driver_id = auth.uid());

drop policy if exists "paquetes_driver_update_propios" on public.paquetes;
create policy "paquetes_driver_update_propios" on public.paquetes for update using (driver_id = auth.uid()) with check (driver_id = auth.uid());

drop policy if exists "paquetes_driver_insert_adhoc" on public.paquetes;
create policy "paquetes_driver_insert_adhoc" on public.paquetes for insert with check (driver_id = auth.uid() and es_imprevisto = true);

drop policy if exists "paquetes_warehouse_por_zona" on public.paquetes;
create policy "paquetes_warehouse_por_zona" on public.paquetes for select using (public.current_role_name() = 'warehouse' and zona = (select zona_asignada from public.perfiles where id = auth.uid()));

drop policy if exists "paquetes_warehouse_update_zona" on public.paquetes;
create policy "paquetes_warehouse_update_zona" on public.paquetes for update using (public.current_role_name() = 'warehouse' and zona = (select zona_asignada from public.perfiles where id = auth.uid()));

drop policy if exists "evidencias_insert_propio" on public.evidencias_escaneo;
create policy "evidencias_insert_propio" on public.evidencias_escaneo for insert with check (usuario_id = auth.uid());

drop policy if exists "evidencias_select_admin_o_dueno" on public.evidencias_escaneo;
create policy "evidencias_select_admin_o_dueno" on public.evidencias_escaneo for select using (usuario_id = auth.uid() or public.current_role_name() in ('admin','executive','warehouse'));

drop policy if exists "inconsistencias_insert_warehouse_admin" on public.inconsistencias;
create policy "inconsistencias_insert_warehouse_admin" on public.inconsistencias for insert with check (public.current_role_name() in ('warehouse','admin'));

drop policy if exists "inconsistencias_select_admin_exec_warehouse" on public.inconsistencias;
create policy "inconsistencias_select_admin_exec_warehouse" on public.inconsistencias for select using (public.current_role_name() in ('admin','executive','warehouse'));

drop policy if exists "inconsistencias_update_admin" on public.inconsistencias;
create policy "inconsistencias_update_admin" on public.inconsistencias for update using (public.current_role_name() in ('admin','warehouse'));

insert into storage.buckets (id, name, public) values ('evidencias', 'evidencias', false) on conflict (id) do nothing;
insert into storage.buckets (id, name, public) values ('firmas', 'firmas', false) on conflict (id) do nothing;

drop policy if exists "evidencias_insert_auth" on storage.objects;
create policy "evidencias_insert_auth" on storage.objects for insert with check (bucket_id = 'evidencias' and auth.role() = 'authenticated' and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists "evidencias_select_auth" on storage.objects;
create policy "evidencias_select_auth" on storage.objects for select using (bucket_id in ('evidencias','firmas') and auth.role() = 'authenticated');

drop policy if exists "firmas_insert_auth" on storage.objects;
create policy "firmas_insert_auth" on storage.objects for insert with check (bucket_id = 'firmas' and auth.role() = 'authenticated' and (storage.foldername(name))[1] = auth.uid()::text);

-- =====================================================================
-- FIN — a continuación ejecuta schema_extension.sql
-- =====================================================================
